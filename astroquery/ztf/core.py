...
class ZtfClass(BaseQuery):
    ...
    def query_region_async(self, coordinates, radius, 
        ...
        response = self._request('GET', self.LCURL, params=request_payload,
    ...
    def _args_to_payload(self, coordinates, radius, **kwargs):
...
def _validate_coord(coordinates):
    ...
    if isinstance(coordinates, (list, tuple)) and len(coordinates) == 2:
        icrscoord = coord.ICRS(Longitude(coordinates[0], unit=u.degree),
                         coord.Latitude(coordinates[1], unit=u.degree))
    else:
        c = commons.parse_coordinates(coordinates)
        if isinstance(c, coord.SkyCoord):
            icrscoord = c.transform_to(coord.ICRS).frame
        elif isinstance(c, coord.BaseCoordinateFrame):
            icrscoord = c.transform_to(coord.ICRS)
